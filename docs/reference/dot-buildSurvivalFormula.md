...
\`multisurvival.b.R\`) to assemble a \`survival::Surv(...) ~ ...\`
formula from raw variable names, with safe escaping of non-syntactic
names via \`.escapeVariableNames()\` and safe parsing via
\`.asSurvivalFormula()\`.
...
- start_var:
  ...
  Character. Start-time variable name (required when \`survival_type =
  "counting"\`).
...
- interaction_terms:
  ...
  (e.g. “ "\`Arm\`:\`Bio\`" “) appended to the right-hand side after
  main effects and before strata.

## Value

A parsed formula object (see \`.asSurvivalFormula()\`).

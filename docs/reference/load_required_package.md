...
Thin wrapper around \`requireNamespace()\` that returns a boolean.
Packages a jamovi module needs must be listed in \`DESCRIPTION\`
\`Imports:\` so they are installed at install time - this helper does
NOT install anything (auto- installing dependencies at runtime is a CRAN
policy violation and a security hazard).
